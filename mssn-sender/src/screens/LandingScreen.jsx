  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", background: '#fff', color: '#0a0a0a' }}>
      ...
      <nav style={{
        ...
        borderBottom: '0.5px solid #e8e8e4', padding: '0 48px', height: 64,
        ...
        <div className="lp-nav-actions" style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => onNavigate('login')} style={{
            ...
            borderRadius: 8, border: '1px solid #e8e8e4', background: 'transparent', color: '#0a0a0a'
          }}>Sign in</button>
          <button onClick={() => onNavigate('login')} style={{
            fontSize: 14, fontWeight: 600, cursor: 'pointer', padding: '8px 18px',
            borderRadius: 8, background: '#0a0a0a', color: '#fff', border: 'none'
      ...
      {menuOpen && (
        <div style={{ background: '#fff', borderBottom: '0.5px solid #e8e8e4', padding: '16px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          {[['#features','Features'],['#how-it-works','How it works'],['#pricing','Pricing']].map(([href, label]) => (
            <a key={href} href={href} onClick={() => setMenuOpen(false)} style={{ fontSize: 15, color: '#0a0a0a', textDecoration: 'none', fontWeight: 500 }}>{label}</a>
          ))}
          <button onClick={() => onNavigate('login')} style={{ fontSize: 15, fontWeight: 600, color: '#fff', padding: '10px 0', background: '#0a0a0a', borderRadius: 8, border: 'none', cursor: 'pointer' }}>
            Get started free
          </button>
        </div>
      )}
      ...
      <section style={{ padding: '120px 24px 80px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <h1 style={{ fontSize: 'clamp(40px, 7vw, 64px)', fontWeight: 700, letterSpacing: '-2px', lineHeight: 1.08, maxWidth: 800, margin: '0 0 24px' }}>
          WhatsApp outreach,<br />done <span style={{ color: '#25D366' }}>properly.</span>
        </h1>

        <p style={{ fontSize: 19, color: '#666', maxWidth: 520, lineHeight: 1.6, margin: '0 0 40px' }}>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'center', marginBottom: 16 }}>
          <button onClick={() => onNavigate('login')} style={{ fontSize: 15, fontWeight: 600, color: '#fff', padding: '14px 28px', borderRadius: 8, background: '#0a0a0a', border: 'none', cursor: 'pointer' }}>
            Start sending free
          </button>
          <a href="#how-it-works" style={{ fontSize: 15, fontWeight: 600, color: '#0a0a0a', padding: '14px 28px', borderRadius: 8, background: '#fff', border: '1px solid #0a0a0a', textDecoration: 'none' }}>
            See how it works
          </a>
        </div>
        <p style={{ fontSize: 13, color: '#999', marginBottom: 64 }}>No credit card required. Connect in 30 seconds.</p>

        <div style={{ width: '100%', maxWidth: 900, background: '#f5f5f3', borderRadius: 16, border: '0.5px solid #e8e8e4', minHeight: 400, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <img src="/images/hero-mockup.png" alt="BulkIt dashboard" style={{ width: '100%', borderRadius: 16, display: 'block' }}
            onError={e => e.target.style.display='none'} />
        </div>
      </section>

      {/* SOCIAL PROOF */}
      <div style={{ background: '#fafaf8', borderTop: '0.5px solid #e8e8e4', borderBottom: '0.5px solid #e8e8e4', padding: '20px 48px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {['MSSN Unilag','Student Unions','Campus Mosques','NGOs','Business Associations'].map(name => (
            <span key={name} style={{ fontSize: 12, fontWeight: 500, color: '#555', background: '#fff', border: '0.5px solid #e8e8e4', borderRadius: 999, padding: '5px 12px' }}>{name}</span>
          ))}
        </div>
      </div>
      ...
      <section style={{ background: '#fff', padding: '80px 48px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 1, background: '#e8e8e4', borderRadius: 16, overflow: 'hidden', maxWidth: 900, margin: '0 auto' }} className="lp-stats">
          {[['2,000+','Recipients per campaign'],['98%','Delivery rate on valid numbers'],['30s','Time to first message']].map(([num, label]) => (
            <div key={label} style={{ background: '#fff', padding: '48px 32px', textAlign: 'center' }}>
              <div style={{ fontSize: 56, fontWeight: 700, letterSpacing: '-2px', lineHeight: 1 }}>{num}</div>
              <div style={{ fontSize: 14, color: '#888', marginTop: 10 }}>{label}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="features" style={{ background: '#fafaf8', padding: '80px 48px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <h2 style={{ fontSize: 'clamp(28px,4vw,40px)', fontWeight: 700, letterSpacing: '-1px', marginBottom: 48, maxWidth: 600 }}>Everything your org needs to communicate at scale</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }} className="lp-features">
            {[
              { title: 'Paste and send instantly', desc: 'Drop numbers in any format. BulkIt auto-formats to international standard and sends within seconds.', icon: <svg width="22" height="22" viewBox="0 0 22 22" fill="none"><path d="M20 2L9 13M20 2L14 20L9 13M20 2L2 9L9 13" stroke="#0a0a0a" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg> },
              { title: 'CSV bulk upload', desc: 'Upload your full member list with names, phone numbers, and groups. Send to all or filter by subgroup.', icon: <svg width="22" height="22" viewBox="0 0 22 22" fill="none"><rect x="3" y="3" width="16" height="16" rx="3" stroke="#0a0a0a" strokeWidth="1.5"/><path d="M3 9h16M3 15h16M9 3v16" stroke="#0a0a0a" strokeWidth="1.5"/></svg> },
              { title: 'Name personalization', desc: 'Use {{name}} to address every member individually. Every message feels personal, never like a mass blast.', icon: <svg width="22" height="22" viewBox="0 0 22 22" fill="none"><circle cx="11" cy="8" r="4" stroke="#0a0a0a" strokeWidth="1.5"/><path d="M3 20c0-4 3.582-7 8-7s8 3 8 7" stroke="#0a0a0a" strokeWidth="1.5" strokeLinecap="round"/></svg> },
              { title: 'Anti-ban protection', desc: 'Smart 3–6 second delays between messages protect your account. Automatic pause every 50 messages.', icon: <svg width="22" height="22" viewBox="0 0 22 22" fill="none"><path d="M11 2L4 5.5v6c0 4.4 3.1 8.5 7 9.5 3.9-1 7-5.1 7-9.5v-6L11 2z" stroke="#0a0a0a" strokeWidth="1.5" strokeLinejoin="round"/><path d="M8 11l2 2 4-4" stroke="#25D366" strokeWidth="1.5" strokeLinecap="round"/></svg> },
              { title: 'Live delivery tracking', desc: 'Watch sends happen in real time. Full per-recipient report showing delivered, failed, and error reasons.', icon: <svg width="22" height="22" viewBox="0 0 22 22" fill="none"><path d="M4 18V10M9 18V6M14 18V12M19 18V4" stroke="#0a0a0a" strokeWidth="1.5" strokeLinecap="round"/></svg> },
              { title: 'Your own WhatsApp number', desc: 'Connect any WhatsApp number via QR scan. Recipients see your real number — not a random shortcode.', icon: <svg width="22" height="22" viewBox="0 0 22 22" fill="none"><path d="M16 14.5c-1 1-1 2-2 2-2.5 0-6.5-4-8.5-8.5 0-1 1-1 2-2L9 3.5c.5-.5.5-1.5 0-2L7 0c-.5-.5-1.5-.5-2 0C2.5 2.5 1 5 1 8.5c0 6.5 5.5 12 12 12 3.5 0 6-1.5 8.5-4 .5-.5.5-1.5 0-2L19.5 12c-.5-.5-1.5-.5-2 0L16 14.5z" stroke="#0a0a0a" strokeWidth="1.5" strokeLinejoin="round"/></svg> },
            ].map(f => (
              <div key={f.title} style={{ background: '#fff', border: '0.5px solid #e8e8e4', borderRadius: 16, padding: 32, boxShadow: '0 1px 2px rgba(0,0,0,0.04)' }}>
                <div style={{ width: 44, height: 44, background: '#f5f5f3', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20 }}>{f.icon}</div>
      ...
      <section id="pricing" style={{ background: '#fafaf8', padding: '80px 48px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, maxWidth: 760, margin: '0 auto' }} className="lp-pricing">
          <div style={{ background: '#fff', border: '0.5px solid #e8e8e4', borderRadius: 16, padding: 36, textAlign: 'left' }}>
            ...
            <button onClick={() => onNavigate('login')} style={{ width: '100%', fontSize: 14, fontWeight: 600, color: '#0a0a0a', padding: '12px 0', border: '1.5px solid #0a0a0a', borderRadius: 8, background: 'transparent', cursor: 'pointer' }}>
              Get started free
            </button>
          </div>
          ...
          <div style={{ background: '#0a0a0a', borderRadius: 16, padding: 36, textAlign: 'left' }}>
            ...
            <button onClick={() => onNavigate('login')} style={{ width: '100%', fontSize: 14, fontWeight: 600, color: '#fff', padding: '12px 0', background: '#25D366', borderRadius: 8, border: 'none', cursor: 'pointer' }}>
              Start free trial
            </button>
          </div>
        </div>
      ...
      <section style={{ background: '#0a0a0a', padding: '100px 48px', textAlign: 'center' }}>
        <h2 style={{ fontSize: 'clamp(32px,5vw,52px)', fontWeight: 700, color: '#fff', letterSpacing: '-1.5px', marginBottom: 16 }}>Ready to reach every member?</h2>
        <p style={{ fontSize: 17, color: '#666', marginBottom: 40 }}>Join organizations already using BulkIt.</p>
        <button onClick={() => onNavigate('login')} style={{ fontSize: 16, fontWeight: 600, color: '#fff', padding: '16px 40px', borderRadius: 10, background: '#25D366', border: 'none', cursor: 'pointer' }}>
          Start sending free
        </button>
      ...
      {/* FOOTER */}
      <footer style={{ background: '#0a0a0a', borderTop: '0.5px solid #1c1c1c', padding: '32px 48px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <Logo size={28} dark />
      ...
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          .lp-features, .lp-pricing, .lp-stats { grid-template-columns: 1fr !important; }
